...
models.lasso.complete <- ml_eval(model.matrix.complete, dat_log, rep = rep, k = k)
...
names(extract.coefs.complete) <- str_replace_all(names(extract.coefs.complete), "Alter", "age")
names(extract.coefs.complete) <- str_replace_all(names(extract.coefs.complete), "Stadium", "AJCC stage")
names(extract.coefs.complete) <- str_replace_all(names(extract.coefs.complete), "hsa\\.", "")
names(extract.coefs.complete) <- str_replace_all(names(extract.coefs.complete), "\\.", "-")
names(extract.coefs.complete) <- str_replace_all(names(extract.coefs.complete), "mir", "miR")

# calculate percentages
feat.freq.complete <- data.frame(sort(extract.coefs.complete/100, decreasing = TRUE)) %>% 
  setNames(c("coef", "freq"))

signif_names <- str_replace_all(colnames(model.matrix.subset("signif", data = dat_log)), "hsa.mir.", "miR-")
signif_names <- str_replace_all(signif_names, "\\.", "-")
axes_cols <- ifelse(feat.freq.complete$coef %in% signif_names, "red", "black")

# plot important features
svg("Results/feature_importance.svg",  width = 15, height = 6)
ggplot(data = feat.freq.complete, aes(coef, freq, fill = ifelse(freq > 0.5, "red", "blue"))) +
  geom_bar(stat = "identity",  color = "black") +
  xlab("") +
  ylab("fraction of cv-models using this feature") +
  theme_bw() +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, color = axes_cols),
...
allmirnas <- dat %>% select(contains("hsa")) %>% colnames() %>% str_replace_all("hsa-mir-", "miR-")
modelmirnas <- feat.freq.complete %>% filter(str_detect(coef, "miR")) %>% .$coef

allmirnas[!allmirnas %in% modelmirnas]
...
models.lasso.relaxedLasso <- ml_eval(model.matrix.relaxedLasso, dat_log, rep = rep, k = k)
...
models.lasso.baseline <- ml_eval(model.matrix.baseline, dat_log, rep = rep, k = k)
...
models.lasso.signif <- ml_eval(modelM, dat_log, rep = rep, k = k)
...
models.lasso.miRNA <- ml_eval(model.matrix.miRNA, dat_log, rep = rep, k = k)
...
models.lasso.relaxed.miRNA <- ml_eval(model.matrix.relaxed.miRNA, dat_log, rep = 10, k = 10)
...
# models.lasso.signifMirna <- readRDS("models/models.lasso.signifMirna.rds")
...
dat_compare[dat_compare$results == "train.inner",]
...
#Plot CV AUC
svg("Results/ROC_curve_outer_loop.svg",  width = 6, height = 5)
plot(out$perf, col="#333333", lty=3, main="10-fold CV AUC (repeated 10 times), outer loop", xlab = "1-Specificity (false positive rate)",
...
feat.final <- names(select(dat_log, c(feat.relaxed$coef, Alter)))
